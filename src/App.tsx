...
const ArticleCard = ({ article, onClick, variant = 'horizontal', onBookmark, isBookmarked, onAuthorClick, categoryIcon }: { 
  ...
  if (variant === 'hero') {
    return (
      <motion.div 
        ...
        whileTap={{ scale: 0.98 }}
        onClick={onClick}
        className="relative h-[240px] w-full rounded-2xl overflow-hidden shadow-xl cursor-pointer group bg-slate-100 african-pattern card-african-hover"
      >
        {article.image && (
          <img 
  ...
  return (
    <motion.div 
      ...
      onClick={onClick}
      className={cn(
        "bg-white rounded-xl overflow-hidden shadow-sm border border-slate-100 cursor-pointer flex transition-all duration-300 african-pattern card-african-hover",
...
const RelatedArticles = ({ currentArticle, articles, onArticleClick, onBookmark, bookmarkedIds, onAuthorClick, categoryIcons }: { 
  ...
  return (
    <section className="mt-16 space-y-8 pt-8 relative">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 decorative-separator-small">◆</div>
      <div className="flex items-center gap-3">
        ...
        {isMenuOpen && (
          ...
          >
            ...
            <nav className="flex flex-col gap-6">
              ...
              <div className="decorative-separator-small my-2 opacity-50">◆</div>
...
En vous inscrivant à notre newsletter, vous acceptez de recevoir périodiquement des courriels d'actualité. Vous pouvez vous désabonner à tout moment via le lien présent en bas de chaque email. Les données collectées pour la newsletter (votre email) ne sont utilisées que pour cet envoi et ne sont jamais transmises à des tiers à des fins commerciales sans votre consentement explicite.
...
`}</ReactMarkdown>
      ...
      <div className="decorative-separator">◆</div>
      <footer className={cn(
        "border-t py-12 md:py-20 transition-colors african-pattern bg-fixed",
        isDarkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"
      )}>
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="space-y-6">
            ...
            <p className="text-[10px] text-slate-400 leading-tight">
              Données collectées via <span onClick={() => navigateTo('privacy')} className="text-primary hover:underline cursor-pointer font-bold">politique de confidentialité</span>.
            </p>
